...
function workTypeLabel(t: string) {
  return t === "short" ? "Short" : t === "series" ? "Serial" : t === "tv" ? "TV" : "Feature";
}

export default function PublicScreenwriterPortfolioPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await fetch(`${base}/public/screenwriter-portfolio/${encodeURIComponent(slug)}`);
  ...
  const sendInquiry = async () => {
    ...
    try {
      const base = getBackendApiUrl();
      const res = await fetch(`${base}/public/screenwriter-portfolio/${encodeURIComponent(slug)}/inquiry`, {
        method: "POST",
  ...
  if (!portfolio) {
    return (
      <main className="min-h-screen bg-[#0a0a0a] px-4 py-16 text-center">
        ...
        <Link href="/screenwriter/features" className="mt-4 inline-block text-sm text-[#e05252]">
          Back
        </Link>
      </main>
    );
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] text-[#F0EBE0]">
      <div className="mx-auto max-w-xl px-4 py-10 space-y-6">
        <Link href="/screenwriter/features" className="text-sm font-semibold text-[#e05252]">
          ← Screenwriter
        </Link>
        ...
        <div className="rounded-2xl border border-[#242424] bg-[#161616] p-5">
          <div className="flex items-start gap-4">
            <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-[#e05252] font-serif text-xl font-bold text-zinc-950">
            ...
            <div>
              ...
              {stats?.verified && (
                <p className="mt-1 text-xs font-semibold text-[#e05252]">✓ Verified writer</p>
              )}
            </div>
          </div>
          <div className="mt-5 grid grid-cols-4 gap-2 text-center">
            ...
            <div>
              <div className="font-serif text-lg font-bold">{stats?.scriptsSold ?? 0}</div>
        ...
        {portfolio.about && (
          <div>
            ...
            <div className="rounded-2xl border border-[#242424] bg-[#161616] p-4 text-sm">{portfolio.about}</div>
          </div>
        )}
        ...
        {active && (
          <div className="rounded-2xl border border-[#333] bg-[#121212] p-4 space-y-2">
            <h3 className="font-serif text-xl">{active.title}</h3>
            <p className="text-sm text-[#909090]">
              {workTypeLabel(active.workType)} · {active.genre} · {active.status}
            </p>
            {active.logline && <p className="text-sm">{active.logline}</p>}
            {active.sampleText && (
            ...
            <button type="button" className="text-xs text-[#e05252]" onClick={() => setActive(null)}>
              Close
            </button>
          </div>
        )}
        ...
        <div className="grid gap-2">
          <button
            ...
            className="rounded-xl bg-[#e05252] px-4 py-3 text-sm font-bold text-white"
          >
            Contact / Hire Writer
          </button>
        </div>
        ...
        {showInquiry && (
          <div className="space-y-3 rounded-2xl border border-[#333] bg-[#161616] p-4">
            <p className="text-sm font-semibold text-[#e05252]">Send Inquiry</p>
            {(
              [
                ["fromName", "Your name"],
                ["fromEmail", "Email"],
              ...
              <input
                key={key}
                className="w-full rounded-xl border border-[#2a2a2a] bg-[#121212] px-3 py-2.5 text-sm outline-none focus:border-[#e05252]"
                placeholder={ph}
            ...
            <textarea
              className="min-h-[90px] w-full rounded-xl border border-[#2a2a2a] bg-[#121212] px-3 py-2.5 text-sm outline-none focus:border-[#e05252]"
              placeholder="Message"
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <button
                type="button"
                className="rounded-xl bg-[#e05252] py-2 text-sm font-bold text-white"
